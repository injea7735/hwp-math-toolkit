...
# 지저분한 잔여물이 안 남는다.
#
# 그 중에서도 "20004 to [[NGD:gtd340400]]"는 DB 전체에서 651번 나온
# 이 워터마크 중 34번이나 완전히 동일한 숫자("20004")를 달고 나온다 —
# 서로 무관한 문제 34개에 우연히 같은 숫자가 실제 수식으로 등장할
# 확률은 사실상 0이라, 이건 실제 문제 내용이 아니라 추출 파이프라인이
# 남긴 별도의 잔여 아티팩트로 보는 게 맞다. 그래서 "to" 바로 앞의
# 리터럴 "20004"도 같이 벗겨낸다(다른 숫자는 실제 수식일 수 있으니
# 건드리지 않는다).
_WATERMARK_TOKEN = re.compile(
    r"\s*(?:20004\s+)?to\s*\[\[NGD:[^\]]*\]\]|\[\[NGD:[^\]]*\]\]"
)
