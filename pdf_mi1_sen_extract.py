...
# 25까지 넓혀서 시도해봤다가 되돌렸다: 수식 위주 제목이 연속되는 구간에서
# 후보를 너무 많이 열어두니 OCR 잡음이 우연히 "더 먼" 엉뚱한 유형과
# 그럴듯하게 비슷해 보여 확신을 갖고 잘못 건너뛰는 사고가 실제로 났다
# (0068이 유형06→유형15로 잘못 재동기화됨). "정보 없어서 위치를 믿는" 게
# "정보를 잘못 믿고 확신에 차서 틀리는" 것보다 낫다 - 6칸을 넘는 드리프트는
# 이 창으로 못 잡고 남은 needs_review/빈 유형으로 남는데, 이건 육안
# 검토·수동 보정으로 처리한다(사람이 원본과 대조해 고친 사례들 참고).
