...
from import_sen_daepyo_types import insert_types, _match_subsection
...
def test_matches_filename_to_canonical_subsection():
    assert _match_subsection('쎈_08 여러 가지 함수의 적분법.hwp') == ('적분법', '여러 가지 적분법')
    assert _match_subsection('쎈_09 치환적분법과 부분적분법.hwp') == ('적분법', '정적분')
    assert _match_subsection('쎈_01 수열의 극한.hwp') == ('수열의 극한', '수열의 극한')


def test_unmatched_filename_raises(session, chapter):
    with pytest.raises(ValueError):
        insert_types(session, '알 수 없는 파일.hwp', [])
...
def test_creates_problem_type_with_real_title_and_order(session, chapter):
    ...
    created = insert_types(session, '쎈_01 수열의 극한.hwp', entries)
...
def test_reuses_existing_subsection_from_another_source(session, chapter):
    ...
    insert_types(session, '쎈_09 치환적분법과 부분적분법.hwp', entries)
...
def test_idempotent_rerun_creates_no_duplicates(session, chapter):
    ...
    insert_types(session, '쎈_01 수열의 극한.hwp', entries)
    created_again = insert_types(session, '쎈_01 수열의 극한.hwp', entries)
