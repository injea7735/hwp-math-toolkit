"""
pdf_mi1_sen_extract.extract_problems()로 뽑은 쎈수학 미적분Ⅰ 문제를
models.Problem에 넣는다. import_sen_problems.py와 거의 같은 구조지만,
미적분1은 대표문제 HWP 인덱스가 없어 정답(answer)을 채울 소스가 없다 -
다른 쎈수학 과목들도 처음부터 정답이 있던 게 아니라 대표문제만 HWP 정답을
갖고 있었으므로, 이 차이는 스키마상 항상 nullable이던 필드가 이 과목에서는
전부 비어 있다는 정도의 차이일 뿐이다.

ProblemType은 import_mi1_sen_types.py가 이미 실제 유형명으로 만들어 뒀으므로
...
# import_sen_problems.py와 같은 기준 - 번호 OCR 실패(파일명이 col{열}-{순번}
# 형태), 비정상적으로 큰 크롭(문제 여러 개가 한 이미지에 뭉침).
...
def insert_problems(session: Session, problems: list[Mi1Problem], pdf_path: str | None = None) -> int:
    ...
    for p in problems:
        ...
        if ptype is None:
            ...
            if ptype is None:
                raise ValueError(f'ProblemType 없음: {code} - import_mi1_sen_types.py를 먼저 실행하세요')
...
if __name__ == '__main__':
    ...
    from pdf_mi1_sen_extract import MI1_SUBSECTION_ORDER, TOC_PAGE_INDICES, extract_problems
